...
from historical_data import load_to_gbq

load_dotenv()

def momentum_trading():
  apca_api = tradeapi.REST(
  ...
  open = apca_api.get_clock().is_open

  if not open:
    print('Markets are not open today.')
    return
  ...
  sql_hist = 'SELECT symbol, close, date FROM kerdos.equity_dataset.nyse_quotes limit 125'

  print('Loading last 125 days stocks data from BigQuery...')
  ...
  print('Latest date from data: {}'.format(current_data_date))

  # Get the current positions from alpaca and create a df
  print('Loading current positions from Alpaca API...')
  ...
  print('Current portfolio value: {}'.format(portfolio_value))

  # Calculate the momentum and select the stocks to buy
  # Set the variables for the momentum trading strategy
  momentum_window = 125
  minimum_momentum = 40
  print('Chosen momentum window: {}. Chosen minium window: {}'.format(momentum_window, minimum_momentum))
  ...
  def momentum_score(ts):
    ...
    annualized_slope = (
        np.power(np.exp(regress[0]), 252) - 1) * 100
  ...
  ).apply(momentum_score, raw=True).reset_index(level=0, drop=True)

  # Get the top momentum stocks for the period
  # Set the portfolio size we want
  portfolio_size = 10
  print('Chosen portfolio size: {}'.format(portfolio_size))
  ...
  def get_momentum_stocks(df, date, portfolio_size, cash):
    ...
    df_buy = df_buy.loc[df_buy['date']
                        == date].sort_values(by='symbol')
  ...
  print('Stocks to buy: {}'.format(df_buy))
  ...
  print('Stocks to sell: {}'.format(sell_list))
  ...
  def sell_stocks(df, df_pf, sell_list, date):
    ...
    # Check to see if there are any stocks in the current ones to buy that are not in the current portfolio. It's possible there may not be any
    if df_sell_price.shape[0] > 0:
        ...
        # Merge with the current pf to get the number of shares we bought initially so we know how many to sell
        df_buy_shares = df_pf[[
          'symbol',
          'qty'
        ]]
  ...
  def stock_diffs(df_sell, df_pf, df_buy):
    ...
    if df_stock_diff.shape[0] > 0:
        ...
        df_stock_diff['share_amt_change'] = df_stock_diff['qty_x'] - \
            df_stock_diff['qty_y']
  ...
  if df_sell_final is not None:
      ...
      try:
        for symbol, qty in list(zip(symbol_list, qty_list)):
          print('Passing sell order for symbol {}. Quantity: {}'.format(symbol, qty))
          apca_api.submit_order(
            symbol=symbol,
            qty=qty,
            side='sell',
            type='market',
  ...
  if df_buy_new is not None:
    ...
    try:
      for symbol, qty in list(zip(symbol_list, qty_list)):
        print('Passing buy order for symbol {}. Quantity: {}'.format(symbol, qty))
        apca_api.submit_order(
          symbol=symbol,
          qty=qty,
          side='buy',
